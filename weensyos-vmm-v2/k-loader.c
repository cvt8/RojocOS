...
static int program_load_segment(proc* p, const elf_program* ph,
    ...
    for (uintptr_t addr = va; addr < end_mem; addr += PAGESIZE) {
        log_printf("program_load_segment(pid %d): assign address %p\n", p->p_pid, addr);

        int r1 = assign_physical_page(addr, p->p_pid);
        
        if (r1 < 0) {
            console_printf(CPOS(22, 0), 0xC000, "program_load_segment(pid %d): r1 < 0, can't assign address %p\n", p->p_pid, addr);
            return -1;
        }
        
        int r2 = virtual_memory_map(p->p_pagetable, addr, addr, PAGESIZE,
                                  PTE_P | PTE_W | PTE_U, allocator);
        
        if (r2 < 0) {
            console_printf(CPOS(22, 0), 0xC000, "program_load_segment(pid %d): r2 < 0, can't assign address %p\n", p->p_pid, addr);
            return -1;
        }
